...
class Instagram
{
	...
	public static function setCurlOpts(array $opts)
	{
	...
	{
		if ($this->sessionUsername == null || $this->sessionPassword == null) {
			return (['status'=>7,'message'=>"User credentials not provided"]);
		}
	...
	public function firstStep($response, $cookies) {
		...
		if ( preg_match( '/window._sharedData\s\=\s(.*?)\;<\/script>/', $response->raw_body, $matches ) ) {
			...
			return [
				'status'=>3,
				'message'=>'Checkpoint alcaçado, escolha algumas das opções abaixo',
				'url'=>$url,
				'headers'=>$headers,
				'choices'=>$choices
			];
